...
@section('admin_content')
  ...
  @if(isset($testCategory->id))
    <form action="{{url('admin/updateQuestionBankCategory')}}" method="POST" id="submitForm">
    {{ method_field('PUT') }}
    <input type="hidden" name="category_id" id="category_id" value="{{$testCategory->id}}">
  @else
   <form action="{{url('admin/createQuestionBankCategory')}}" method="POST" id="submitForm">
  @endif
    {{ csrf_field() }}
...
<script type="text/javascript">
  function searchCategory(){
    ...
    if(category){
      $.ajax({
        method:'POST',
        url: "{{url('admin/isQuestionBankCategoryExist')}}",
