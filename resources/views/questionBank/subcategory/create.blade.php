...
@section('admin_content')
  ...
  @if(isset($testSubcategory->id))
    <form action="{{url('admin/updateQuestionBankSubCategory')}}" method="POST" enctype="multipart/form-data" id="submitForm">
    {{method_field('PUT')}}
    <input type="hidden" name="subcat_id" id="subCategory_id" value="{{$testSubcategory->id}}">
  @else
    <form action="{{url('admin/createQuestionBankSubCategory')}}" method="POST" enctype="multipart/form-data" id="submitForm">
  @endif
    {{ csrf_field() }}
      ...
      @if(isset($testSubcategory->id))
        @if(count($testCategories) > 0)
          @foreach($testCategories as $testCategory)
            @if($testSubcategory->question_bank_category_id == $testCategory->id)
...
<script type="text/javascript">
  function searchSubCategory(){
    ...
    if(category && subcategory){
      $.ajax({
        method:'POST',
        url: "{{url('admin/isQuestionBankSubCategoryExist')}}",
