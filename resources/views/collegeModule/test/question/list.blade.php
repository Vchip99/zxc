...
@section('dashboard_content')
	...
	<div  class="admin_div">
		<form id="questionForm" action="{{url('college/'.Session::get('college_user_url').'/showQuestions')}}" method="POST">
			{{csrf_field()}}
	...
	<div class="form-group row ">
      <div id="addQuestionDiv">
        <a id="addQuestion" href="{{url('college/'.Session::get('college_user_url').'/createQuestion')}}" type="button" class="btn btn-primary" style="float: right; margin-right: 12px;" title="Add New Question">Add New Question</a>&nbsp;&nbsp;
      </div>
    </div>
	<div>
	    <table id="collegeListQuestions">
	      	...
	      	<tbody>
	      		@if(count($questions) > 0)
			        @foreach($questions as $index => $question)
			        	@if(isset($question->id))
					        <tr>
					          ...
					          <td>{!! $question->name !!}</td>
					          <td>
					          	@if($question->created_by == Auth::User()->id || (4 ==  Auth::User()->user_type || 5 ==  Auth::User()->user_type))
					            <a href="{{url('college/'.Session::get('college_user_url').'/question')}}/{{$question->id}}/edit"><img src="{{asset('images/edit1.png')}}" width='30' height='30' title="Edit Question" /></a>
					            @endif
					          </td>
					          <td>
					          	@if($question->created_by == Auth::User()->id || (4 ==  Auth::User()->user_type || 5 ==  Auth::User()->user_type))
					          	<a id="{{$question->id}}" onclick="confirmDelete(this);"><img src="{{asset('images/delete2.png')}}" width='30' height='30' title="Delete Question" />
					              <form id="deleteQuestion_{{$question->id}}" action="{{url('college/'.Session::get('college_user_url').'/deleteQuestion')}}" method="POST" style="display: none;">
					                  {{ csrf_field() }}
					                  {{ method_field('DELETE') }}
			        ...
			        @endforeach
	    		@else
	    			<tr><td colspan="4">No questions are created.</td></tr>
	    		@endif
	      	</tbody>
	    </table>
  	</div>
    ...
    function selectSubcategory(ele){
  	...
  	function selectSubject(ele){
	    ...
	    if( 0 < catId && 0 < subcatId ){
	      $.ajax({
	              method: "POST",
	              url: "{{url('getCollegeSubjectsByCatIdBySubcatIdByUserType')}}",
