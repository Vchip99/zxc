	...
	<div class="container ">
		...
		@php
			if(Session::has('search_college_selected_category')){
				$searchSelectedCategoryId = Session::get('search_college_selected_category');
			} else {
				$searchSelectedCategoryId = 0;
			}
			if(Session::has('search_college_selected_subcategory')){
				$searchSelectedSubcategoryId = Session::get('search_college_selected_subcategory');
			} else {
				$searchSelectedSubcategoryId = 0;
			}
			if(Session::has('search_college_selected_subject')){
				$searchSelectedSubjectId = Session::get('search_college_selected_subject');
			} else {
				$searchSelectedSubjectId = 0;
			}
			if(Session::has('search_college_selected_paper')){
				$searchSelectedPaperId = Session::get('search_college_selected_paper');
			} else {
				$searchSelectedPaperId = 0;
			}
			if(Session::has('search_college_selected_section')){
				$searchSelectedSectionId = Session::get('search_college_selected_section');
			} else {
				$searchSelectedSectionId = 0;
			}
			if(Session::has('search_college_question_bank_category')){
				$searchSelectedBankCategoryId = Session::get('search_college_question_bank_category');
			} else {
				$searchSelectedBankCategoryId = 0;
			}
			if(Session::has('search_college_question_bank_subcategory')){
				$searchSelectedBankSubcategoryId = Session::get('search_college_question_bank_subcategory');
			} else {
		...
		@endphp
	  	...
	  	<form id="questionForm" action="{{url('college/'.Session::get('college_user_url').'/useCollegeQuestionBank')}}" method="POST">
			{{csrf_field()}}
		...
		<form id="questionBankForm" action="{{url('college/'.Session::get('college_user_url').'/exportCollegeQuestionBank')}}" method="POST">
			{{csrf_field()}}
			<div id="all-result">
					      	...
					      	<tbody>
					      		@if(count($questions) > 0)
							        @foreach($questions as $index => $question)
									    ...
									    @endif
							        @endforeach
							        <input type="hidden" name="selected_college_category" id="selected_college_category" value="">
							        <input type="hidden" name="selected_college_subcategory" id="selected_college_subcategory" value="">
							        <input type="hidden" name="selected_college_subject" id="selected_college_subject" value="">
							        <input type="hidden" name="selected_college_paper" id="selected_college_paper" value="">
...
<script type="text/javascript">
	...
	function setInputs(){
	    ...
	    document.getElementById('selected_college_category').value = category;
	    document.getElementById('selected_college_subcategory').value = subcategory;
	    document.getElementById('selected_college_subject').value = subject;
	    document.getElementById('selected_college_paper').value = paper;
	...
	function selectBankSubcategory(ele){
	    ...
	    if( 0 < id ){
	      $.ajax({
	              method: "POST",
	              url: "{{url('getCollegeQuestionBankSubCategories')}}",
	              data: {id:id}
	          })
	          .done(function( msg ) {
