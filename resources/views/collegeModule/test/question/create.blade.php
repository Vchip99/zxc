...
@section('dashboard_content')
	...
	@php
		if(Session::has('selected_college_category')){
			$selectedCategoryId = Session::get('selected_college_category');
		} else {
			$selectedCategoryId = 0;
		}
		if(Session::has('selected_college_subcategory')){
			$selectedSubCategoryId = Session::get('selected_college_subcategory');
		} else {
			$selectedSubCategoryId = 0;
		}
		if(Session::has('selected_college_subject')){
			$selectedSubjectId = Session::get('selected_college_subject');
		} else {
			$selectedSubjectId = 0;
		}
		if(Session::has('selected_college_paper')){
			$selectedPaperId = Session::get('selected_college_paper');
		} else {
			$selectedPaperId = 0;
		}
		if(Session::has('selected_college_section')){
			$selectedSectionId = Session::get('selected_college_section');
		} else {
			$selectedSectionId = 0;
		}
		if(Session::has('selected_college_question_type')){
			$selectedQuestionType = Session::get('selected_college_question_type');
		} else {
			$selectedQuestionType = 1;
		}
		if(Session::has('next_college_question_no')){
			$nextQuestionNo = Session::get('next_college_question_no');
		} else {
			$nextQuestionNo = 1;
		}
		if(Session::has('last_college_common_data')){
			$lastCommonData = Session::get('last_college_common_data');
		} else {
			$lastCommonData = '';
		}
	@endphp
	<div id="main ">
		<ul id ="ul">
			...
			@endif
	  		...
	  		@if($prevQuestionId > 0)
	  			<li title="Prev Question"><a class="btn" id="prev_ques" href="{{url('college/'.Session::get('college_user_url').'/question')}}/{{$prevQuestionId}}/edit">Prev Question</a></li>
	  		@else
	  			<li title="No Prev Question"><a class="btn" id="prev_ques">No Prev Question</a></li>
	  		@endif
	  		@if($nextQuestionId > 0)
		  		<li title="Next Question"><a class="btn" id="next_ques" href="{{url('college/'.Session::get('college_user_url').'/question')}}/{{$nextQuestionId}}/edit">Next Question</a></li>
	  		@elseif(( $prevQuestionId > 0 || null == $prevQuestionId ) && null == $nextQuestionId )
	  			<li title="Add Question"><a class="btn" id="next_ques" href="{{url('college/'.Session::get('college_user_url').'/createQuestion')}}">Add Question </a></li>
	  		@else
		...
		@if(isset($testQuestion->id))
			<form id="createForm" name="updateQuestion" action="{{url('college/'.Session::get('college_user_url').'/updateQuestion')}}" method="POST" role="form" data-toggle="validator">
			{{method_field('PUT')}}
			<input type="hidden" name="question_id" id="question_id" value="{{$testQuestion->id}}">
		@else
			<form id="createForm" name="createQuestion" action="{{url('college/'.Session::get('college_user_url').'/createQuestion')}}" method="POST">
		@endif
		{{csrf_field()}}
		<div class="container admin_div">
			    ...
			    @if(count($testCategories) > 0 && isset($testQuestion->id))
			    	@foreach($testCategories as $testCategory)
		              @if( $testCategory->id == $testQuestion->category_id )
		              	...
		              	<input class="form-control" type="hidden" name="category" id="category" value="{{$testQuestion->category_id}}">
		              @endif
		            @endforeach
			    @else
		        	...
		        	@if(count($testSubCategories) > 0 && isset($testQuestion->id))
			        	@foreach($testSubCategories as $testSubCategory)
				            @if($testQuestion->subcat_id == $testSubCategory->id)
				                ...
				                <input class="form-control" type="hidden" name="subcategory" id="subcategory" value="{{$testQuestion->subcat_id}}">
				            @endif
			          	@endforeach
			        @else
	          		...
	          		@if(count($testSubjects) > 0 && isset($testQuestion->id) )
	          			@foreach($testSubjects as $testSubject)
		          			@if($testQuestion->subject_id == $testSubject->id)
	            			 	...
	            			 	<input class="form-control" type="hidden" name="subject" id="subject" value="{{$testQuestion->subject_id}}">
		            		@endif
		          		@endforeach
	          		@else
	    			...
	    			@if(count($papers) > 0 && isset($testQuestion->id))
	    				@foreach($papers as $paper)
				            @if($testQuestion->paper_id == $paper->id)
				                ...
				                <input class="form-control" type="hidden" name="paper" id="paper" value="{{$testQuestion->paper_id}}">
				            @endif
			          	@endforeach
		    ...
		    <div class="form-group row ">
			    ...
			    <div class="col-sm-10" title="@if(isset($testQuestion->id)) Update @else Add @endif Question">
			      	@if(isset($testQuestion->id) && $testQuestion->subject->created_by == Auth::User()->id)
			        	<input class="btn btn-primary" type="button" value="Update" onClick="checkCkeditor();">
			        @elseif(empty($testQuestion->id))
			        	<input class="btn btn-primary" type="button" value="Add" onClick="checkCkeditor();">
			        @endif
...
<script type="text/javascript">
    ...
    function showHideCommonData(ele){
  	...
  	function selectSubject(ele){
	    ...
	    if( 0 < catId && 0 < subcatId ){
	      $.ajax({
	              method: "POST",
	              url: "{{url('getCollegeSubjectsByCatIdBySubcatIdByUser')}}",
	...
	function selectPaper(ele){
    	...
    	if( 0 < subjectId ){
	      	$.ajax({
	             	method: "POST",
	              	url: "{{url('getCollegePapersBySubjectId')}}",
  	...
  	function getQuestionCount(categoryId,subcategoryId,paperId,subjectId,section_type){
  		...
  		if( 0 < categoryId && 0 < subcategoryId && 0 < paperId && 0 < subjectId && false == isNaN(questionId)){
	      	$.ajax({
	             	method: "POST",
	              	url: "{{url('getCurrentQuestionCount')}}",
  	...
  	function selectPaperSection(){
		...
		if( paperId > 0  && subjectId > 0){
	      	$.ajax({
	             	method: "POST",
	              	url: "{{url('getPaperSectionsByPaperId')}}",
