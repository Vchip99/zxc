...
@section('dashboard_content')
  ...
  @if(isset($testSubcategory->id))
    <form action="{{url('college/'.Session::get('college_user_url').'/updateSubCategory')}}" method="POST" enctype="multipart/form-data" id="submitForm">
    {{method_field('PUT')}}
    <input type="hidden" name="subcat_id" id="subCategory_id" value="{{$testSubcategory->id}}">
  @else
    <form action="{{url('college/'.Session::get('college_user_url').'/createSubCategory')}}" method="POST" enctype="multipart/form-data" id="submitForm">
  @endif
    {{ csrf_field() }}
  ...
  <div class="form-group row">
      <div class="offset-sm-2 col-sm-3" title="Submit">
        @if(isset($testSubcategory->id) && $testSubcategory->created_by == Auth::User()->id)
