...
@section('dashboard_content')
  ...
  @if(isset($paper->id))
    <form action="{{url('college/'.Session::get('college_user_url').'/updatePaper')}}" method="POST" id="submitForm">
    {{method_field('PUT')}}
    <input type="hidden" name="paper_id" id="paper_id" value="{{$paper->id}}">
  @else
    <form action="{{url('college/'.Session::get('college_user_url').'/createPaper')}}" method="POST" id="submitForm">
  @endif
    {{ csrf_field() }}
  ...
  <div class="form-group row">
      <div class="offset-sm-2 col-sm-3" title="Submit">
        @if(isset($paper->id) && $paper->subject->created_by == Auth::User()->id)
