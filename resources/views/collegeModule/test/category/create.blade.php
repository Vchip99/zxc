...
@section('dashboard_content')
  ...
  @if(isset($testCategory->id))
    <form action="{{url('college/'.Session::get('college_user_url').'/updateCategory')}}" method="POST" id="submitForm">
    {{ method_field('PUT') }}
    <input type="hidden" name="category_id" id="category_id" value="{{$testCategory->id}}">
  @else
   <form action="{{url('college/'.Session::get('college_user_url').'/createCategory')}}" method="POST" id="submitForm">
  @endif
    {{ csrf_field() }}
