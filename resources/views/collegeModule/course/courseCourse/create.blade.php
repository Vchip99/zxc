...
@section('dashboard_content')
  ...
  @if(isset($course->id))
    <form action="{{url('college/'.Session::get('college_user_url').'/updateCourseCourse')}}" method="POST" enctype="multipart/form-data" id="submitForm">
    {{method_field('PUT')}}
    <input type="hidden" id="course_id" name="course_id" value="{{$course->id}}">
  @else
    <form action="{{url('college/'.Session::get('college_user_url').'/createCourseCourse')}}" method="POST" enctype="multipart/form-data" id="submitForm">
  @endif
    {{ csrf_field() }}
    ...
    <div class="form-group row">
        <div class="offset-sm-2 col-sm-3" title="Submit">
          @if(isset($course->id) && $course->created_by == Auth::User()->id)
