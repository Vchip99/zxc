...
@section('dashboard_content')
  ...
  @if(isset($timeTable->id))
    <form action="{{url('college/'.Session::get('college_user_url').'/updateExamTimeTable')}}" method="POST" id="submitForm" enctype="multipart/form-data">
    {{ method_field('PUT') }}
    <input type="hidden" id="time_table_id" name="time_table_id" value="{{$timeTable->id}}">
  @else
   <form action="{{url('college/'.Session::get('college_user_url').'/createExamTimeTable')}}" method="POST" id="submitForm" enctype="multipart/form-data">
  @endif
    {{ csrf_field() }}
