...
@section('dashboard_content')
  ...
  @if(isset($project->id))
    <form action="{{url('college/'.Session::get('college_user_url').'/updateVkitProject')}}" method="POST" enctype="multipart/form-data" id="submitForm">
    {{method_field('PUT')}}
    <input type="hidden" name="project_id" id="project_id" value="{{$project->id}}">
  @else
    <form action="{{url('college/'.Session::get('college_user_url').'/createVkitProject')}}" method="POST" enctype="multipart/form-data" id="submitForm">
  @endif
    {{ csrf_field() }}
    ...
    <div class="form-group row">
      <div class="offset-sm-2 col-sm-3" title="Submit">
        @if(isset($project->id) && $project->created_by == Auth::User()->id)
