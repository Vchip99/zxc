...
@section('dashboard_content')
  ...
  @if(isset($vkitCategory->id))
    <form action="{{url('college/'.Session::get('college_user_url').'/updateVkitCategory')}}" method="POST" id="submitForm">
    {{ method_field('PUT') }}
    <input type="hidden" name="category_id" id="category_id" value="{{$vkitCategory->id}}">
  @else
    <form action="{{url('college/'.Session::get('college_user_url').'/createVkitCategory')}}" method="POST" id="submitForm">
  @endif
    {{ csrf_field() }}
