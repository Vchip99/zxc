          ...
          <div class="comment-meta">
            ...
            <div class="collapse replyComment" id="replyToEpisode{{$liveVideo->id}}">
              <form action="{{ url('createLiveCourseComment')}}" method="POST" id="createLiveCourseComment">
                ...
                <button type="button" class="btn btn-default" data-id="replyToEpisode{{$liveVideo->id}}" onclick="cancleReply(this);" title="Cancle">
                              ...
                              <div class="dropdown pull-right">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                                  @if(Auth::user()->id == $comment->user_id)
                                    <li><a id="{{$comment->id}}" data-comment_id="{{$comment->id}}" data-video_id="{{$liveVideo->id}}" onclick="confirmCommentDelete(this);">Delete</a></li>
                                  @endif
                                  @if(Auth::user()->id == $comment->user_id)
                              ...
                              @endif
                                ...
                                <div class="more" id="editCommentHide_{{$comment->id}}">{!! $comment->body !!}</div>
                                  <div class="form-group hide" id="editCommentShow_{{$comment->id}}" >
                                    ...
                                    <button class="btn btn-primary" data-comment_id="{{$comment->id}}" data-video_id="{{$liveVideo->id}}" onclick="updateComment(this);">Update</button>
                              ...
                              <div class="collapse replyComment" id="replyToComment{{$comment->id}}">
                                  <div class="form-group">
                                    <label for="subcomment">Your Sub Comment</label>
                                      <textarea name="subcomment" class="form-control" id="subcomment_{{$liveVideo->id}}_{{$comment->id}}" rows="3"></textarea>
                                  </div>
                                  <button type="button" class="btn btn-default" onclick="confirmSubmitReplytoComment(this);" data-comment_id="{{$comment->id}}" data-video_id="{{$liveVideo->id}}" >Send</button>
...
@section('footer')
    ...
    $.each(sortedArray, function(idx, obj) {
      if(false == $.isEmptyObject(obj)){
        ...
        if( userId == obj.user_id ){
          ...
          if( userId == obj.user_id ){
            editDeleteInnerHtml += '<li><a id="'+obj.id+'" data-comment_id="'+obj.id+'" data-video_id="'+obj.live_course_video_id+'" onclick="confirmCommentDelete(this);">Delete</a></li>';
          }
          if( userId == obj.user_id ){
        ...
        divUpdateComment.innerHTML = '<textarea class="form-control" name="comment" id="comment'+ obj.id +'" rows="3">'+ obj.body+'</textarea><button class="btn btn-primary" data-comment_id="'+ obj.id +'" data-video_id="'+ obj.live_course_video_id +'" onclick="updateComment(this);">Update</button><button type="button" class="btn btn-default" id="'+ obj.id +'" onclick="cancleComment(this);">Cancle</button>';
        ...
        if( commentLikesCount[obj.id] && commentLikesCount[obj.id]['user_id'][userId]){
          spanCommenInnerHtml +='<i id="comment_like_'+obj.id+'" data-video_id="'+obj.live_course_video_id+'" data-comment_id="'+obj.id+'" data-dislike="1" class="fa fa-thumbs-up" aria-hidden="true" data-placement="bottom" title="remove like"></i>';
          spanCommenInnerHtml +='<span id="like1-bs3">'+ Object.keys(commentLikesCount[obj.id]['like_id']).length +'</span>';
        } else {
          spanCommenInnerHtml +='<i id="comment_like_'+obj.id+'" data-video_id="'+obj.live_course_video_id+'" data-comment_id="'+obj.id+'" data-dislike="0" class="fa fa-thumbs-o-up" aria-hidden="true" data-placement="bottom" title="add like"></i>';
        ...
        subCommenDiv.innerHTML = '<div class="form-group"><label for="subcomment">Your Sub Comment</label><textarea name="subcomment" class="form-control" rows="3"  id="subcomment_'+obj.live_course_video_id+'_'+obj.id+'" ></textarea></div><button type="button" class="btn btn-default" onclick="confirmSubmitReplytoComment(this);" data-comment_id="'+obj.id+'" data-video_id="'+obj.live_course_video_id+'" >Send</button><button type="button" class="btn btn-default" data-id="replyToComment'+obj.id+'" onclick="cancleReply(this);">Cancle</button>';
    ...
    if(false == $.isEmptyObject(subcomments)){
      $.each(subcomments, function(idx, obj) {
        ...
        if( userId == obj.user_id || userId == commentUserId){
          ...
          if(  userId == obj.user_id || userId == commentUserId){
            editDeleteInnerHtml += '<li><a id="'+obj.live_course_comment_id+'_'+obj.id+'" onclick="confirmSubCommentDelete(this);"  data-subcomment_id="'+obj.id+'" data-comment_id="'+obj.live_course_comment_id+'" data-video_id="'+obj.live_course_video_id+'">Delete</a></li>';
          }
          if( userId == obj.user_id ){
        ...
        divUpdateSubComment.innerHTML = '<textarea class="form-control" name="comment" id="updateSubComment_'+ obj.id +'" rows="3">'+ obj.body+'</textarea><button class="btn btn-primary"  data-subcomment_id="'+ obj.id +'" data-comment_id="'+ obj.live_course_comment_id +'" data-video_id="'+ obj.live_course_video_id +'" onclick="updateLiveCourseSubComment(this);">Update</button><button type="button" class="btn btn-default" id="'+ obj.id +'" onclick="cancleSubComment(this);">Cancle</button></div></form>';
        ...
        if( subcommentLikesCount[obj.id] && subcommentLikesCount[obj.id]['user_id'][userId]){
          spanSubCommenInnerHtml +='<i id="sub_comment_like_'+obj.id+'" data-video_id="'+obj.live_course_video_id+'" data-comment_id="'+obj.live_course_comment_id+'"  data-sub_comment_id="'+obj.id+'" data-dislike="1" class="fa fa-thumbs-up" aria-hidden="true" data-placement="bottom" title="remove like"></i>';
          spanSubCommenInnerHtml +='<span id="like1-bs3">'+ Object.keys(subcommentLikesCount[obj.id]['like_id']).length +'</span>';
        } else {
          spanSubCommenInnerHtml +='<i id="sub_comment_like_'+obj.id+'" data-video_id="'+obj.live_course_video_id+'" data-comment_id="'+obj.live_course_comment_id+'" data-sub_comment_id="'+obj.id+'" data-dislike="0" class="fa fa-thumbs-o-up" aria-hidden="true" data-placement="bottom" title="add like"></i>';
        ...
        spanSubCommenReplyButton.innerHTML = '<a class="" role="button" data-toggle="collapse" href="#replySubComment'+obj.live_course_comment_id+'-'+obj.id+'" aria-expanded="false" aria-controls="collapseExample">reply</a>';
        ...
        createSubCommenDiv.id = 'replySubComment'+obj.live_course_comment_id+'-'+obj.id;
        ...
        createSubCommenDivInnerHTML += '</div><button class="btn btn-default" onclick="confirmSubmitReplytoSubComment(this);" data-subcomment_id="'+ obj.id +'" data-comment_id="'+ obj.live_course_comment_id +'" data-video_id="'+ obj.live_course_video_id +'" >Send</button><button class="btn btn-default" data-id="replySubComment'+ obj.live_course_comment_id +'-'+ obj.id +'" onclick="cancleReply(this);">Cancle</button>';
  ...
  function confirmSubmit(ele){
    ...
    if(userId > 0){
      ...
      $.ajax({
              method: "POST",
              url: "{{url('createLiveCourseComment')}}",
  ...
  function confirmSubmitReplytoComment(ele){
    ...
    if(0 < userId){
        ...
        $.ajax({
            method: "POST",
            url: "{{url('createLiveCourseSubComment')}}",
  ...
  function updateComment(ele){
    ...
    $.ajax({
        method: "POST",
        url: "{{url('updateLiveCourseComment')}}",
        ...
        buttons: {
              Ok: {
                  ...
                  action: function(){
                    ...
                    $.ajax({
                        method: "POST",
                        url: "{{url('deleteLiveCourseComment')}}",
        ...
        buttons: {
              Ok: {
                  ...
                  action: function(){
                    ...
                    $.ajax({
                        method: "POST",
                        url: "{{url('deleteLiveCourseSubComment')}}",
  ...
  function updateLiveCourseSubComment(ele){
      var commentId = $(ele).data('comment_id');
      var videoId = $(ele).data('video_id');
      var subcommentId = $(ele).data('subcomment_id');
      ...
      $.ajax({
          method: "POST",
          url: "{{url('updateLiveCourseSubComment')}}",
