	...
	<div class="item replySubComment-1">
	  	...
	  	<div class="message" id="subcomment_{{$subcomment->id}}">
	    	...
	    	<div class="dropdown pull-right">
		      	...
		      	<ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
		      		@if(Auth::user()->id == $subcomment->user_id || Auth::user()->id == $comment->user_id)
		                ...
		                <form id="deleteSubComment_{{$parent}}_{{$subcomment->id}}" action="{{ url('deleteLiveCourseSubComment')}}" method="POST" style="display: none;">
		                    {{ csrf_field() }}
		                    {{ method_field('DELETE') }}
	    	...
	    	@endif
		    	...
		    	{!! $subcomment->body !!}
              	...
              	<form action="{{ url('updateLiveCourseSubComment')}}" method="POST" id="formUpdateSubComment{{$subcomment->id}}">
                    {{csrf_field()}}
                    {{ method_field('PUT') }}
	    ...
	    <div class="comment-meta reply-1">
		    ...
		    <div class="collapse replyComment" id="replySubComment{{$parent}}-{{$subcomment->id}}">
              	<form action="{{ url('createLiveCourseSubComment')}}" method="POST" id="formReplyToSubComment{{$parent}}{{$subcomment->id}}">
                	{{csrf_field()}}
