@foreach($comments as $subcomment)
	<div class="item replySubComment-1">
	  	@if(!empty($subcomment->getUser($subcomment->user_id)->photo))
		    ...
		    <div class="collapse replyComment" id="replySubComment{{$subcomment->placement_process_comment_id}}-{{$subcomment->id}}">
                	<div class="form-group">
                  		...
                  		@if(is_object($currentUser) && $currentUser->id != $subcomment->user_id)
	                        <textarea name="subcomment" id="createSubComment_{{$subcomment->id}}" class="form-control" rows="3">@if(is_object($subcomment->getUser($subcomment->user_id))) {{$subcomment->getUser($subcomment->user_id)->name}} @endif</textarea>
                      	@else
