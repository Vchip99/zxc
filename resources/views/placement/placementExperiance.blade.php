...
<head>
  ...
  <title>Placement Experiance |Vchip Technology</title>
  <meta charset="utf-8" />
  ...
  <div class="container">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav ">
        ...
        <li><a href="{{url('placements')}}">Placement <span class="sr-only">(current)</span></a></li>
      </ul>
    </div>
  </div>
</nav>
    ...
    <div class="row">
      ...
      <div class="col-md-3">
        <div class="vchip-right-sidebar mrgn_30_top_btm">
          <h3 class="v_h3_title text-center">Recent Posts</h3>
          <ul class="vchip_list">
            @if(count($placementExperiances) > 0)
              @foreach($placementExperiances as $experiance)
                <li title="{{ $experiance->title }}"><a href="{{url('placementExperiance')}}/{{$experiance->id}}">{{ $experiance->title }}</a></li>
              @endforeach
            @endif
          </ul>
        </div>
