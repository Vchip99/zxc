  ...
  <div class="container">
    ...
    <div id="frame">
      <div id="sidepanel">
        <div id="profile">
          <div class="wrap">
            @if(is_file(Auth::guard('mentor')->user()->photo) || (!empty(Auth::guard('mentor')->user()->photo) && false == preg_match('/userStorage/',Auth::guard('mentor')->user()->photo)))
              <img id="dashboardUserImage" src="{{ asset(Auth::guard('mentor')->user()->photo)}} " class="online" alt="User Image">
            @else
              <img id="dashboardUserImage" src="{{ url('images/user/user1.png')}}" class="online" alt="User Image">
            @endif
            <p>{{Auth::guard('mentor')->user()->name}}</p>
          </div>
        </div>
        <div id="contacts">
          <ul id="contact_list">
            ...
            <li class="contact" id="{{$user['id']}}" data-user_name="{{$user['name']}}" data-user_email="{{$user['email']}}" onclick="showChat(this);">
              <div class="wrap">
                @if(is_file($user['photo']) || (!empty($user['photo']) && false == preg_match('/userStorage/',$user['photo'])))
                  <img id="image_{{$user['id']}}" src="{{ asset($user['photo'])}}" class="online" alt="User Image">
                @else
                  <img id="image_{{$user['id']}}" src="{{ url('images/user/user1.png')}}" class="online" alt="User Image">
                @endif
                <div class="meta">
                  <p class="name">{{$user['name']}} ({{$user['email']}})
                      <strong style="color: red;" id="unreadCount_{{$user['id']}}"></strong>
                  </p>
                </div>
                <input type="hidden" id="message_limit_{{$user['id']}}" value="0">
                <input type="hidden" id="is_scroll_{{$user['id']}}" value="0">
              </div>
            </li>
            @endforeach
            @endif
...
<script type="text/javascript">
  ...
  function showChat(ele){
    ...
    document.getElementById('profile_email').innerHTML = '('+$(ele).data('user_email')+')';
    // add chat window
  ...
  function sendMessage(ele) {
      ...
      if(message != '' && id != ''){
          ...
          $.ajax({
              ...
              dataType: "json",
              data: {'_token':token,'message':message,'sender':parseInt(sender) ,'receiver':receiver, 'chatroomId':chatroomId,'generated_by_mentor':1},
              success:function(messages){
