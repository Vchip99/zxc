		    ...
		    <div class="collapse replyComment" id="replySubComment{{$subcomment->vkit_project_comment_id}}-{{$subcomment->id}}">
                	<div class="form-group">
                  		...
                  		@if(is_object($currentUser) && $currentUser->id != $subcomment->user_id)
	                        <textarea name="subcomment" class="form-control" id="createSubComment_{{$subcomment->id}}" rows="3">@if(is_object($subcomment->getUser($subcomment->user_id))) {{$subcomment->getUser($subcomment->user_id)->name}} @endif</textarea>
                      	@else
