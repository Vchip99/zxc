...
<div class="container admin_div">
  @if(isset($workshopCategory->id))
    <form action="{{url('admin/updateWorkshopCategory')}}" method="POST" id="submitForm">
    {{ method_field('PUT') }}
    <input type="hidden" name="category_id" id="category_id" value="{{$workshopCategory->id}}">
  @else
    <form action="{{url('admin/createWorkshopCategory')}}" method="POST" id="submitForm">
  @endif
      {{ csrf_field() }}
...
<script type="text/javascript">
  function searchCategory(){
    ...
    if(category){
      $.ajax({
        method:'POST',
        url: "{{url('admin/isOnlineWorkshopCategoryExist')}}",
