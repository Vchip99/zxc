            ...
            <div class="col-md-3 mrgn_10_btm" id="showUsers">
              <select class="form-control" id="user" name="user_type" onChange="showResult();" required="true">
                ...
                @if(5 == Auth::user()->user_type)
                  <option value="6" @if(6 == $selectedUserType) selected @endif>TNP</option>
                @endif
              </select>
            </div>
            @endif
          ...
          <div class="col-lg-12" id="all-result">
            <div class="panel panel-info">
              ...
              <div class="panel-body">
                      ...
                      @if(count($users) > 0)
                        @foreach($users as $index => $user)
                          <tr style="overflow-x: auto;">
                            ...
                            <td>
                              {{$collegeDeptNames[$user->college_dept_id]}}
                            </td>
                            <td>{{$user->year}}</td>
                            <td>{{$user->roll_no}}</td>
                            <td>
                ...
                @if(3 == $selectedUserType || 4 == $selectedUserType || 6 == $selectedUserType)
                      ...
                      @if(count($users) > 0)
                        @foreach($users as $index => $user)
                          <tr style="overflow-x: auto;">
                            ...
                            <td>
                              @if((3 == $selectedUserType && 4 == Auth::user()->user_type) || (4 == $selectedUserType && 5 == Auth::user()->user_type))
                                <a href="#deptModal_{{$user->id}}" data-toggle="modal">{{$collegeDeptNames[$user->college_dept_id]}}</a>
                              @else
                                @if(6 == $user->user_type)
                                  -
                                @else
                              ...
                              <div class="modal-dialog modal-sm">
                                <div class="modal-content">
                                  ...
                                  <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                      <a href="{{url('college/'.Session::get('college_user_url').'/lecturerPapers')}}/{{$user->id}}">Lecturer Papers</a>
                                    </div>
                                      <div class="form-group">
                                        <a href="{{url('college/'.Session::get('college_user_url').'/lecturerCourses')}}/{{$user->id}}">Lecturer Course</a>
                                    </div>
                                  </div>
                                  <div class="modal-footer">
                                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                          ...
                          @endif
                          @if((3 == $selectedUserType && 4 == Auth::user()->user_type) || (4 == $selectedUserType && 5 == Auth::user()->user_type))
                            <div class="modal fade in" id="deptModal_{{$user->id}}" role="dialog" style="padding-right: 15px;">
                              <div class="modal-dialog modal-sm">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal">×</button>
                                    <h4 class="modal-title">Assigned Departments</h4>
                                    <b>Note:</b>If remove depatement then data of this user associated with this department will be deleted.
                                  </div>
                                  <form action="{{url('assignDepatementsToUser')}}" method="POST">
                                    {{ csrf_field() }}
                                    <div class="modal-body">
                                      <div class="form-group">
                                        @if(count($collegeAllDeptNames) > 0)
                                          @php
                                            $assignedDepts = explode(',', $user->assigned_dept_ids);
                                          @endphp
                                          @foreach($collegeAllDeptNames as $deptId => $deptName)
                                            @if($user->college_dept_id == $deptId )
                                              <input type="checkbox" name="departments[]" value="{{$deptId}}" checked disabled>{{$deptName}}
                                            @elseif(isset($assignedDepts[$deptId]))
                                              <input type="checkbox" name="departments[]" value="{{$deptId}}" checked>{{$deptName}}
                                            @else
                                              @if(5 == Auth::user()->user_type)
                                                <input type="checkbox" name="departments[]" value="{{$deptId}}" >{{$deptName}}
                                              @else
                                                <input type="checkbox" name="departments[]" value="{{$deptId}}" disabled>{{$deptName}}
                                              @endif
                                            @endif
                                          @endforeach
                                        @endif
                                      ...
                                      <input type="hidden" name="departments[]" value="{{$user->college_dept_id}}">
                                    </div>
                                    <div class="modal-footer">
...
<script type="text/javascript">
  ...
  function showResult(){
    ...
    if(3 == user_type || 4 == user_type || 6 == user_type){
      document.getElementById('div_year').classList.add('hide');
      resetYear();
      showStudents();
    }
    ...
    if(document.getElementById("user")){
      ...
      .done(function( msg ) {
        if(2 == user_type){
          ...
          body.innerHTML = '';
        } else if(3 == user_type || 4 == user_type || 6 == user_type){
          document.getElementById('lectures_hods_record').classList.remove('hide');
          document.getElementById('student-record').classList.add('hide');
