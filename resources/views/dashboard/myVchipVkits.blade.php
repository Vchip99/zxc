    ...
    @foreach($projects as $project)
      <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
        <div class="course-box">
          <a class="img-course-box" href="{{ url('college/'.Session::get('college_user_url').'/vkitproject')}}/{{$project->id}}" target="_blank">
          ...
          <div class="course-box-content" >
             <h4 class="course-box-title " title="{{$project->name}}" data-toggle="tooltip" data-placement="bottom"> <p class="block-with-text"><a href="{{ url('college/'.Session::get('college_user_url').'/vkitproject')}}/{{$project->id}}" target="_blank">{{$project->name}}</a></p></h4>
             <br/>
            ...
            <div class="corse-detail" id="corse-detail-{{$project->id}}">
                <div class="corse-detail-heder">
                  ...
                  <div class="text-center corse-detail-footer" >
                    <a href="{{ url('college/'.Session::get('college_user_url').'/vkitproject')}}/{{$project->id}}" class="btn btn-primary btn-default" target="_blank"> Start Project</a>
                  </div>
              </div>
            </div>
            <div class="course-auther">
              <a href="{{ url('college/'.Session::get('college_user_url').'/vkitproject')}}/{{$project->id}}" target="_blank"><i class="fa fa-long-arrow-right block-with-text" aria-hidden="true" title="{{$project->author}}"> {{$project->author}}</i>
              </a>
            </div>
          </div>
        </div>
    ...
    function renderVkitProjects(msg){
      ...
      if( 0 < msg.length){
        ...
        var firstDiv = document.createElement('div');
          ...
          var url = "{{ url('college/'.Session::get('college_user_url').'/vkitproject')}}/"+ obj.id;
