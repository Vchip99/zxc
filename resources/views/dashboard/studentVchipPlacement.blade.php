...
@section('dashboard_content')
    ...
    <div id="placement" class="">
      <div class="top mrgn_40_btm"">
        <div class="container">
          <div class="row">
            <a href="{{ url('college/'.Session::get('college_user_url').'/studentVchipPlacement')}}" class="btn btn-primary">Vchip Placement</a>&nbsp;
            <a href="{{ url('college/'.Session::get('college_user_url').'/studentCollegePlacement')}}" class="btn btn-default" >College Placement</a>
          </div>
          <br>
          <div class="row">
          ...
          @if(count($students) > 0)
            ...
            <div style="border:1px solid black;">
              <div class="row memberinfo" >
                ...
                <div class="col-md-7">
                  @php
                    if(!empty($student->skill_ids)){
                      $skillArr = explode(',',$student->skill_ids);
                    }else{
                      $skillArr = [];
                    }
                  @endphp
...
<script type="text/javascript">
  ...
  function searchStudent(){
    ...
    if(department && year && student.length > 0){
      $.ajax({
          method: "POST",
          url: "{{url('searchVchipStudentByDeptByYearByName')}}",
  ...
  function showStudents(){
    ...
    if(department&& year){
      $.ajax({
          method: "POST",
          url: "{{url('showVchipPlacementVideoByDepartmentByYear')}}",
  ...
  function renderData(msg){
    ...
    if(Object.keys(msg['users']).length) {
      $.each(msg['users'], function(id, userData) {
          ...
          if(userData.youtube){
            thirdDiv.innerHTML = '<div class="vid">'+userData.youtube+'</div>';
          } else {
          ...
          if(userData.skill_ids){
            var studenSkills = userData.skill_ids.split(',');
