        ...
        @foreach($courses as $course)
          <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
            <div class="course-box">
              <a class="img-course-box" href="{{ url('college/'.Session::get('college_user_url').'/collegeCourseDetails')}}/{{$course->id}}" target="_blank">
              ...
              <div class="course-box-content" >
                 <h4 class="course-box-title " title="{{$course->name}}" data-toggle="tooltip" data-placement="bottom"> <p class="block-with-text"><a href="{{ url('college/'.Session::get('college_user_url').'/collegeCourseDetails')}}/{{$course->id}}" target="_blank">{{$course->name}}</a></p></h4>
                 <div class="categoery">
                   <a  href="{{ url('college/'.Session::get('college_user_url').'/collegeCourseDetails')}}/{{$course->id}}" target="_blank"> {{$course->category}}</a>
                 </div>
                 <br/>
                ...
                <div class="corse-detail" id="corse-detail-{{$course->id}}">
                    <div class="corse-detail-heder">
                      ...
                      <div class="text-center corse-detail-footer" >
                        <a href="{{ url('college/'.Session::get('college_user_url').'/collegeCourseDetails')}}/{{$course->id}}" class="btn btn-primary btn-default" target="_blank"> Start Course</a>
                      </div>
                  </div>
                </div>
                <div class="course-auther">
                  <a href="{{ url('college/'.Session::get('college_user_url').'/collegeCourseDetails')}}/{{$course->id}}" target="_blank"><i class="fa fa-long-arrow-right block-with-text" aria-hidden="true" title="{{$course->author}}"> {{$course->author}}</i>
                  </a>
                </div>
              </div>
            </div>
...
<script type="text/javascript">
  ...
  function renderCourse(msg){
    ...
    if(undefined !== msg['courses'] && 0 < msg['courses'].length) {
      $.each(msg['courses'], function(idx, obj) {
        ...
        var url = "{{ url('college/'.Session::get('college_user_url').'/collegeCourseDetails')}}/"+ obj.id;
  ...
  function selectCourses(ele){
    ...
    if(catId > 0 && subcatId > 0 && userId > 0){
      $.ajax({
        method: "POST",
        url: "{{url('getCollegeCourseByCatIdBySubCatId')}}",
