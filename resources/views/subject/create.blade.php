...
@section('admin_content')
  ...
  @if(isset($subject->id))
    <form action="{{url('admin/updateSubject')}}" method="POST" id="submitForm">
    {{method_field('PUT')}}
    <input type="hidden" name="subject_id" id="subject_id" value="{{$subject->id}}">
  @else
    <form action="{{url('admin/createSubject')}}" method="POST" id="submitForm">
  @endif
    {{ csrf_field() }}
      ...
      <select id="category" class="form-control" name="category" onChange="selectSubcategory(this);" required title="Category">
          ...
          @if(count($testCategories) > 0)
            @foreach($testCategories as $testCategory)
              @if( isset($subject->id) && $subject->test_category_id == $testCategory->id)
