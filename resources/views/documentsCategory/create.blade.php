...
@section('admin_content')
  ...
  @if(isset($documentsCategory->id))
    <form action="{{url('admin/updateDocumentsCategory')}}" method="POST" id="submitForm">
    {{ method_field('PUT') }}
    <input type="hidden" name="category_id" id="category_id" value="{{$documentsCategory->id}}"/>
  @else
   <form action="{{url('admin/createDocumentsCategory')}}" method="POST" id="submitForm">
    @endif
    {{ csrf_field() }}
...
<script type="text/javascript">
  function searchCategory(){
    ...
    if(category){
      $.ajax({
        method:'POST',
        url: "{{url('admin/isDocumentCategoryExist')}}",
