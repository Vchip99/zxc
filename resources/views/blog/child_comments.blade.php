	  	...
	  	<div class="message" id="subcomment_{{$subcomment->id}}">
	    	...
	    	<div class="dropdown pull-right">
		      	...
		      	<ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
		      		@if(Auth::user()->id == $subcomment->user_id || Auth::user()->id == $blog->user_id || Auth::user()->id == $comment->user_id)
		                ...
		                <form id="deleteSubComment_{{$parent}}_{{$subcomment->id}}" action="{{ url('deleteBlogSubComment')}}" method="POST" style="display: none;">
		                    {{ csrf_field() }}
		                    {{ method_field('DELETE') }}
	    	...
	    	@endif
		    	...
		    	{!! $subcomment->body !!}
              	...
              	<form action="{{ url('updateBlogSubComment')}}" method="POST" id="formUpdateSubComment{{$subcomment->id}}">
                    {{csrf_field()}}
                    {{ method_field('PUT') }}
                	<div class="form-group hide" id="editSubCommentShow_{{$subcomment->id}}" >
                  		<textarea class="form-control" name="comment" rows="3">@php $string = preg_replace ("/<b>(.*?)<\/b>/i", "", $subcomment->body); $string = preg_replace('/\s+/', ' ',$string); $string = preg_replace('/\t+/', ' ',$string); $string = trim($string); @endphp {!! $string !!}</textarea>
                  		<input type="hidden" name="blog_id" value="{{$blog->id}}">
                  		<input type="hidden" name="comment_id" value="{{$comment->id}}">
                  		<input type="hidden" name="subcomment_id" value="{{$subcomment->id}}">
	    ...
	    <div class="comment-meta reply-1">
	      	<span id="sub_cmt_like_{{$subcomment->id}}" >
                @if( isset($subcommentLikesCount[$subcomment->id]) && isset($subcommentLikesCount[$subcomment->id]['user_id'][$currentUser]))
                     <i id="sub_comment_like_{{$subcomment->id}}" data-blog_id="{{$comment->blog_id}}" data-comment_id="{{$comment->id}}" data-sub_comment_id="{{$subcomment->id}}" data-dislike='1' class="fa fa-thumbs-up" aria-hidden="true" data-placement="bottom" title="remove like"></i>
                     <span id="like1-bs3">{{count($subcommentLikesCount[$subcomment->id]['like_id'])}}</span>
                @else
                     <i id="sub_comment_like_{{$subcomment->id}}" data-blog_id="{{$comment->blog_id}}" data-comment_id="{{$comment->id}}"  data-sub_comment_id="{{$subcomment->id}}" data-dislike='0' class="fa fa-thumbs-o-up" aria-hidden="true" data-placement="bottom" title="add like"></i>
		    ...
		    <div class="collapse replyComment" id="replySubComment{{$parent}}-{{$subcomment->id}}">
              	<form action="{{ url('createBlogSubComment')}}" method="POST" id="formReplyToSubComment{{$parent}}{{$subcomment->id}}">
                	{{csrf_field()}}
                	<div class="form-group">
                  		<label for="subcomment">Your Sub Comment</label>
                  		@if(is_object(Auth::user()) && Auth::user()->id != $subcomment->user_id)
	                        <textarea name="comment" class="form-control" rows="3">@if(is_object($user->find($subcomment->user_id))) {{$user->find($subcomment->user_id)->name}} @endif</textarea>
                      	@else
