...
<div class="container admin_div">
  @if(isset($motivationalSpeechCategory->id))
    <form action="{{url('admin/updateMotivationalSpeechCategory')}}" method="POST" id="submitForm">
    {{ method_field('PUT') }}
    <input type="hidden" name="category_id" id="category_id" value="{{$motivationalSpeechCategory->id}}">
  @else
    <form action="{{url('admin/createMotivationalSpeechCategory')}}" method="POST" id="submitForm">
  @endif
      {{ csrf_field() }}
      <div class="form-group row  @if ($errors->has('category')) has-error @endif">
        <label class="col-sm-2 col-form-label" for="category">Speaker Name:</label>
        <div class="col-sm-3">
...
<script type="text/javascript">
  function searchCategory(){
    ...
    if(category){
      $.ajax({
        method:'POST',
        url: "{{url('admin/isMotivationalSpeechCategoryExist')}}",
