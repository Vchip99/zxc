         	...
         	<div class="media-body">
	         	...
	         	<div class="comment-meta">
                      ...
                      @if( isset($commentLikesCount[$comment->id]) && isset($commentLikesCount[$comment->id]['user_id'][$currentUser]))
                           <i id="comment_like_{{$comment->id}}" data-post_id="{{$comment->client_all_post_id}}" data-comment_id="{{$comment->id}}" data-dislike='1' class="fa fa-thumbs-up" aria-hidden="true" data-placement="bottom" title="remove like"></i>
                           <span id="like1-bs3">{{count($commentLikesCount[$comment->id]['like_id'])}}</span>
                      @else
                           <i id="comment_like_{{$comment->id}}" data-post_id="{{$comment->client_all_post_id}}" data-comment_id="{{$comment->id}}" data-dislike='0' class="fa fa-thumbs-o-up" aria-hidden="true" data-placement="bottom" title="add like"></i>
	              ...
	              <div class="collapse replyComment" id="replyToComment{{$comment->id}}">
	                <form action="{{ url('createClientAllChildComment')}}" method="POST" id="formReplyToComment{{$comment->id}}">
		                 {{csrf_field()}}
		                ...
		                <input type="hidden" name="client_all_post_id" value="{{$post->id}}">
		                <input type="hidden" name="episode_id" value="{{$episodeId}}" >
		                <input type="hidden" name="project_id" value="0" >
		                <input type="hidden" name="client_id" value="{{ (is_object(Auth::guard('clientuser')->user()))?Auth::guard('clientuser')->user()->client_id: NULL }}" >
		                <button type="button" class="btn btn-default" onclick="confirmSubmit(this);" id="formReplyToComment{{$comment->id}}">Send</button>
	              	</form>
	              </div>
	            </div>
	         	@if(count( $comment->children) > 0)
		        	@include('client.front.onlineCourses.comments', ['comments' => $comment->children, 'parent' => (int) $comment->id, 'user' => $user, 'episodeId' => $video->id])
	          	@endif
	      </div>
	    </div>
	    </div>
