	...
	<div class="item replySubComment-1">
	  	...
	  	<div class="message">
	    	...
	    	<div class="dropdown pull-right">
		      	...
		      	<ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
		      		@if(Auth::guard('clientuser')->user()->id == $subcomment->user_id || Auth::guard('clientuser')->user()->id == $comment->user_id)
				        <li><a id="{{$parent}}_{{$subcomment->id}}" onclick="confirmSubCommentDelete(this);">Delete</a></li>
				        </a>
		                <form id="deleteSubComment_{{$parent}}_{{$subcomment->id}}" action="{{ url('deleteClientCourseSubComment')}}" method="POST" style="display: none;">
		                    {{ csrf_field() }}
		                    {{ method_field('DELETE') }}
		            ...
		            @endif
		            @if(Auth::guard('clientuser')->user()->id == $subcomment->user_id)
	    	...
	    	@endif
		    	...
		    	{!! $subcomment->body !!}
              	...
              	<form action="{{ url('updateClientCourseSubComment')}}" method="POST" id="formUpdateSubComment{{$subcomment->id}}">
                    {{csrf_field()}}
                    {{ method_field('PUT') }}
	    ...
	    <div class="comment-meta reply-1">
		    ...
		    <div class="collapse replyComment" id="replySubComment{{$parent}}-{{$subcomment->id}}">
              	<form action="{{ url('createClientCourseSubComment')}}" method="POST" id="formReplyToSubComment{{$parent}}{{$subcomment->id}}">
                	{{csrf_field()}}
                	<div class="form-group">
                  		<label for="subcomment">Your Sub Comment</label>
                  		@if(is_object(Auth::guard('clientuser')->user()) && Auth::guard('clientuser')->user()->id != $subcomment->user_id)
