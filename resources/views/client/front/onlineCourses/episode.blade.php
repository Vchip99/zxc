                      ...
                      @foreach($comments as $comment)
                        <div class="item" id="showComment_{{$comment->id}}">
                          @if(is_file($comment->getClientUser($subdomainName, $comment->user_id)->photo) && true == preg_match('/clientUserStorage/',$comment->getClientUser($subdomainName, $comment->user_id)->photo))
                            <img src="{{ asset($comment->getClientUser($subdomainName, $comment->user_id)->photo) }}" alt="User Image" />
                          @elseif(!empty($comment->getClientUser($subdomainName, $comment->user_id)->photo) && false == preg_match('/clientUserStorage/',$comment->getClientUser($subdomainName, $comment->user_id)->photo))
                            <img src="{{ $comment->getClientUser($subdomainName, $comment->user_id)->photo }}" alt="User Image" />
                          @else
                          ...
                          <div class="message">
                            ...
                            <div class="dropdown pull-right">
                              ...
                              <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                                @if($currentUser->id == $comment->user_id)
                                  <li><a id="{{$comment->id}}" onclick="confirmCommentDelete(this);" data-comment_id="{{$comment->id}}" data-video_id="{{$video->id}}">Delete</a></li>
                                @endif
                                @if($currentUser->id == $comment->user_id)
                            ...
                            @endif
                              <a class="SubCommentName">{{ $comment->getClientUser($subdomainName, $comment->user_id)->name }}</a>
                            ...
                            <div class="collapse replyComment" id="replyToComment{{$comment->id}}">
                                <div class="form-group">
                                  <label for="subcomment">Your Sub Comment</label>
                                    <textarea name="subcomment" class="form-control" id="subcomment_{{$video->id}}_{{$comment->id}}" rows="3"></textarea>
                                </div>
                                <button class="btn btn-default" onclick="confirmSubmitReplytoComment(this);" data-comment_id="{{$comment->id}}" data-video_id="{{$video->id}}">Send</button>
                                <button class="btn btn-default" data-id="replyToComment{{$comment->id}}" onclick="cancleReply(this);">Cancle</button>
                            </div>
                          </div>
                        </div>
                        @if(count( $comment->children ) > 0)
                          @include('client.front.onlineCourses.comments', ['comments' => $comment->children, 'parent' => $comment->id, 'subdomainName' => $subdomainName, 'videoId' => $video->id,'currentUser' => $currentUser])
                        @endif
                      @endforeach
                    @endif
                  </div>
...
@section('footer')
    ...
    $.each(sortedArray, function(idx, obj) {
      if(false == $.isEmptyObject(obj)){
        ...
        if( userId == obj.user_id ){
          ...
          if( userId == obj.user_id ){
            editDeleteInnerHtml += '<li><a id="'+obj.id+'" data-comment_id="'+obj.id+'" data-video_id="'+obj.client_online_video_id+'" onclick="confirmCommentDelete(this);">Delete</a></li>';
          }
          if( userId == obj.user_id ){
        ...
        divUpdateComment.innerHTML = '<textarea class="form-control" name="comment" id="comment_'+ obj.id +'" rows="3">'+ obj.body+'</textarea><button class="btn btn-primary" data-comment_id="'+ obj.id +'" data-video_id="'+ obj.client_online_video_id +'" onclick="updateComment(this);">Update</button><button type="button" class="btn btn-default" id="'+ obj.id +'" onclick="cancleComment(this);">Cancle</button>';
        ...
        if( commentLikesCount[obj.id] && commentLikesCount[obj.id]['user_id'][userId]){
          spanCommenInnerHtml +='<i id="comment_like_'+obj.id+'" data-video_id="'+obj.client_online_video_id+'" data-comment_id="'+obj.id+'" data-dislike="1" class="fa fa-thumbs-up" aria-hidden="true" data-placement="bottom" title="remove like"></i>';
          spanCommenInnerHtml +='<span id="like1-bs3">'+ Object.keys(commentLikesCount[obj.id]['like_id']).length +'</span>';
        } else {
          spanCommenInnerHtml +='<i id="comment_like_'+obj.id+'" data-video_id="'+obj.client_online_video_id+'" data-comment_id="'+obj.id+'" data-dislike="0" class="fa fa-thumbs-o-up" aria-hidden="true" data-placement="bottom" title="add like"></i>';
        ...
        subCommenDiv.innerHTML = '<div class="form-group"><label for="subcomment">Your Sub Comment</label><textarea name="subcomment" class="form-control" rows="3"  id="subcomment_'+obj.client_online_video_id+'_'+obj.id+'" ></textarea></div><button class="btn btn-default" onclick="confirmSubmitReplytoComment(this);" data-comment_id="'+obj.id+'" data-video_id="'+obj.client_online_video_id+'" >Send</button><button type="button" class="btn btn-default" data-id="replyToComment'+obj.id+'" onclick="cancleReply(this);">Cancle</button>';
    ...
    if(false == $.isEmptyObject(subcomments)){
      $.each(subcomments, function(idx, obj) {
        ...
        if( userId == obj.user_id || userId == commentUserId){
          ...
          if(  userId == obj.user_id || userId == commentUserId){
            editDeleteInnerHtml += '<li><a id="'+obj.client_course_comment_id+'_'+obj.id+'" onclick="confirmSubCommentDelete(this);"  data-subcomment_id="'+obj.id+'" data-comment_id="'+obj.client_course_comment_id+'" data-video_id="'+obj.client_online_video_id+'">Delete</a></li>';
          }
          if( userId == obj.user_id ){
        ...
        divUpdateSubComment.innerHTML = '<textarea class="form-control" name="comment" id="updateSubComment_'+ obj.id +'" rows="3">'+ obj.body+'</textarea><button class="btn btn-primary"  data-subcomment_id="'+ obj.id +'" data-comment_id="'+ obj.client_course_comment_id +'" data-video_id="'+ obj.client_online_video_id +'" onclick="updateCourseSubComment(this);">Update</button><button type="button" class="btn btn-default" id="'+ obj.id +'" onclick="cancleSubComment(this);">Cancle</button></div></form>';
        ...
        if( subcommentLikesCount[obj.id] && subcommentLikesCount[obj.id]['user_id'][userId]){
          spanSubCommenInnerHtml +='<i id="sub_comment_like_'+obj.id+'" data-video_id="'+obj.client_online_video_id+'" data-comment_id="'+obj.client_course_comment_id+'"  data-sub_comment_id="'+obj.id+'" data-dislike="1" class="fa fa-thumbs-up" aria-hidden="true" data-placement="bottom" title="remove like"></i>';
          spanSubCommenInnerHtml +='<span id="like1-bs3">'+ Object.keys(subcommentLikesCount[obj.id]['like_id']).length +'</span>';
        } else {
          spanSubCommenInnerHtml +='<i id="sub_comment_like_'+obj.id+'" data-video_id="'+obj.client_online_video_id+'" data-comment_id="'+obj.client_course_comment_id+'" data-sub_comment_id="'+obj.id+'" data-dislike="0" class="fa fa-thumbs-o-up" aria-hidden="true" data-placement="bottom" title="add like"></i>';
        ...
        createSubCommenDiv.id = 'replySubComment'+obj.client_course_comment_id+'-'+obj.id;
        ...
        createSubCommenDivInnerHTML += '</div><button class="btn btn-default" onclick="confirmSubmitReplytoSubComment(this);" data-subcomment_id="'+ obj.id +'" data-comment_id="'+ obj.client_course_comment_id +'" data-video_id="'+ obj.client_online_video_id +'" >Send</button><button class="btn btn-default" data-id="replySubComment'+ obj.client_course_comment_id +'-'+ obj.id +'" onclick="cancleReply(this);">Cancle</button>';
  ...
  function updateComment(ele){
    ...
    $.ajax({
        method: "POST",
        url: "{{url('updateClientCourseComment')}}",
  ...
  function updateCourseSubComment(ele){
      ...
      $.ajax({
          method: "POST",
          url: "{{url('updateClientCourseSubComment')}}",
  ...
  function confirmSubmit(ele){
    ...
    if(userId > 0 && comment && videoId > 0){
      ...
      $.ajax({
              method: "POST",
              url: "{{url('createClientCourseComment')}}",
        ...
        buttons: {
              Ok: {
                  ...
                  action: function(){
                    ...
                    $.ajax({
                        method: "POST",
                        url: "{{url('deleteClientCourseComment')}}",
        ...
        buttons: {
              Ok: {
                  ...
                  action: function(){
                    ...
                    $.ajax({
                        method: "POST",
                        url: "{{url('deleteClientCourseSubComment')}}",
  ...
  $(document).ready(function() {
    ...
    $(document).on("click", "i[id^=comment_like_]", function(e) {
        ...
        var userId = parseInt(document.getElementById('user_id').value);
         if( isNaN(userId)) {
          ...
          $.ajax({
              method: "POST",
              url: "{{url('likeClientCourseVideoComment')}}",
    ...
    $(document).on("click", "i[id^=sub_comment_like_]", function(e) {
        ...
        if( isNaN(userId)) {
          ...
          $.ajax({
              method: "POST",
              url: "{{url('likeClientCourseVideoSubComment')}}",
