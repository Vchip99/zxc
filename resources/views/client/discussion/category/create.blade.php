...
@section('dashboard_content')
  ...
  @if(isset($category->id))
    <form action="{{url('updateDiscussionCategory')}}" method="POST" id="submitForm">
    {{ method_field('PUT') }}
    <input type="hidden" name="category_id" id="category_id" value="{{$category->id}}">
  @else
    <form action="{{url('createDiscussionCategory')}}" method="POST" id="submitForm">
  @endif
    {{ csrf_field() }}
...
<script type="text/javascript">
  function searchCategory(){
    ...
    if(category){
      $.ajax({
        method:'POST',
        url: "{{url('isClientDiscussionCategoryExist')}}",
