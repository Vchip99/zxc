...
@section('dashboard_content')
  ...
  <div class="container admin_div">
    <form action="{{url('createAllTestCategory')}}" method="POST" id="submitCategoryForm">
      {{ csrf_field() }}
  ...
  <br>
  <div class="container admin_div">
    <form action="{{url('createAllTestSubCategory')}}" method="POST" enctype="multipart/form-data" id="submitSubCategoryForm">
      {{ csrf_field() }}
      <div class="form-group row @if ($errors->has('category')) has-error @endif">
        <label class="col-sm-2 col-form-label">Category Name:</label>
        <div class="col-sm-3">
          <select class="form-control" id="select_category" name="category" required title="Category">
  ...
  <br>
  <div class="container admin_div">
    <form action="{{url('createAllTestSubject')}}" method="POST" id="submitSubjectForm">
      {{ csrf_field() }}
      <div class="form-group row @if ($errors->has('category')) has-error @endif">
        <label class="col-sm-2 col-form-label">Category Name:</label>
        <div class="col-sm-3">
          <select id="subject_category" class="form-control" name="category" onChange="selectSubjectSubcategory(this);" required title="Category">
              <option value="">Select Category</option>
              @if(count($testCategories) > 0)
                @foreach($testCategories as $testCategory)
                    <option value="{{$testCategory->id}}">{{$testCategory->name}}</option>
  ...
  <br>
  <div class="container admin_div">
    <form action="{{url('createAllTestPaper')}}" method="POST" id="submitPaperForm">
      {{ csrf_field() }}
      <div class="form-group row @if ($errors->has('category')) has-error @endif">
        <label class="col-sm-2 col-form-label">Category Name:</label>
        <div class="col-sm-3">
...
<script type="text/javascript">
  function searchCategory(){
    ...
    if(category){
      $.ajax({
        method:'POST',
        url: "{{url('isClientTestCategoryExist')}}",
  ...
  function selectSubjectSubcategory(ele){
    id = parseInt($(ele).val());
    if( 0 < id ){
      $.ajax({
          method: "POST",
