...
@section('dashboard_content')
	...
	<div class="container">
  	...
  	<div class="container admin_div">
	    <form action="{{url('createAllCourseCategory')}}" method="POST" id="submitCategoryForm">
		    {{ csrf_field() }}
	...
	<br>
	<div class="container admin_div">
    	<form action="{{url('createAllCourseSubCategory')}}" method="POST" id="submitSubCategoryForm">
			{{ csrf_field() }}
	  		<div class="form-group row @if ($errors->has('category')) has-error @endif">
  	...
  	<br>
  	<div class="container admin_div">
    	<form action="{{url('createAllCourseCourse')}}" method="POST" enctype="multipart/form-data" id="submitCourseForm">
    		{{ csrf_field() }}
		    <div class="form-group row @if ($errors->has('category')) has-error @endif">
		      <label class="col-sm-2 col-form-label">Category Name:</label>
		      <div class="col-sm-3">
		        <select id="course_category" class="form-control" name="category" onChange="selectCoursSubcategory(this);" required title="Category">
		            <option value="">Select Category</option>
		            @if(count($categories) > 0)
		              	@foreach($categories as $category)
		                 	<option value="{{$category->id}}">{{$category->name}}</option>
...
<script type="text/javascript">
  	...
  	function selectCoursSubcategory(ele){
