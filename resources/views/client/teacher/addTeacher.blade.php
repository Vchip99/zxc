...
@section('module_title')
  ...
  <section class="content-header">
    <h1> Add Teachers </h1>
    <ol class="breadcrumb">
      <li><i class="fa fa-graduation-cap"></i> Teacher Info </li>
...
@section('dashboard_content')
  ...
  <div class="container admin_div">
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-3">
        <a class="btn btn-primary cust-btn" href="{{asset('Download Add Teachers File.xlsx')}}" download data-toggle="tooltip" data-placement="bottom">Download Add Teachers File</a>
      </div>
      <div class="col-sm-3">
        <button id="submitButton" type="submit" class="btn btn-primary cust-btn1" data-toggle="modal" data-target="#modelAddTeachers" >Upload Teachers</button>
      </div>
      <div class="modal fade" id="modelAddTeachers" >
        <div class="modal-dialog" role="document" >
          <div class="modal-content" >
            <div class="modal-header">
              <h5 class="modal-title model-title">Add Teachers</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <form id="uploadClientTeachersForm" action="{{url('uploadClientTeachers')}}" method="POST" enctype="multipart/form-data">
              {{csrf_field()}}
              <div class="modal-body">
                <div class="form-group row">
                  <label class="col-sm-3 col-form-label">Add Teachers:</label>
                  <div class="col-sm-3">
                    <input type="file" class="model-input" id="upload_teachers" name="teachers" required="true">
                  </div>
                </div>
              </div>
              <div class="modal-footer">
                <button class="btn btn-primary cust-btn1" data-dismiss="modal" onclick="uploadTeachers();">Add Teachers</button>
    ...
    <div id="signUpEmailDiv">
      <form action="{{url('addEmailTeachers')}}" method="POST" id="addEmailTeachers">
        {{ csrf_field() }}
        <div id="allTeachers">
          <div class="form-group row" id="1">
            <label class="col-sm-2 col-form-label" >Teacher Details <sup>*<sup></label>
            <div class="col-sm-2">
              <input type="text" class="form-control" id="name_1" name="name_1" value="" placeholder="Teacher Name" required>
            </div>
            <div class="col-sm-2">
              <input type="text" class="form-control" name="phone_1" value="" placeholder="Mobile number(10 digit)" pattern="[0-9]{10}" required>
            </div>
            <div class="col-sm-2">
        ...
        <div class="form-group row">
          <div class="offset-sm-2 col-sm-3">
            <button id="addTeacher" class="btn btn-primary cust-btn2" onclick="event.preventDefault(); addNewTeacher();" >Add Teacher</button>
    ...
    <div class="hide" id="signUpPhoneDiv">
      <form action="{{url('addMobileTeacher')}}" method="POST">
        {{ csrf_field() }}
        <div class="form-group row">
          <label class="col-sm-2 col-form-label" for="name">Teacher Name <sup>*<sup></label>
          <div class="col-sm-3">
            <input id="signUpNameInput" type="text" class="form-control" name="name" value="" placeholder="Teacher Name" required>
          </div>
        </div>
        <div class="form-group row" >
...
<script type="text/javascript">
  $('#addEmailTeachers').on('submit', function() {
    if(false == $('#addEmailTeachers').get(0).checkValidity()) {
      return false;
    }
  ...
  function addNewTeacher(){
    var allTeachers = document.getElementById('allTeachers');
    var latestCount = ($('#allTeachers input').length/4) + 1;
    ...
    phoneDiv.innerHTML = '<input type="text" class="form-control" name="phone_'+latestCount+'" value="" placeholder="Mobile number(10 digit)" pattern="[0-9]{10}" required>';
    ...
    eleImg.setAttribute("onclick", 'removeElement(\'allTeachers\','+latestCount+')');
    eleDiv.appendChild(eleImg);
    allTeachers.appendChild(eleDiv);
  ...
  function uploadTeachers(){
    if(document.getElementById('upload_teachers').value.length > 0){
      document.getElementById('uploadClientTeachersForm').submit();
    } else {
      alert('please add a teachers file.');
          return false;
    }
  }
</script>
