...
@section('dashboard_content')
  <div class="container admin_div">
   <form action="{{url('createClientGalleryImage')}}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
        ...
        @if(isset($galleryImage->id))
          @if(count($galleryTypes))
            @foreach($galleryTypes as $galleryType)
              @if($galleryImage->client_gallery_type_id == $galleryType->id)
    ...
    <div class="form-group row">
      ...
      @else
        <a class="btn btn-primary" href="{{url('manageGalleryImages')}}" style="width: 90px !important;">Back</a>
      @endif
      </div>
    </div>
    </form>
