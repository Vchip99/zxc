	...
	<div class="item replySubComment-1">
        ...
        @endif
	  	<div class="message" id="subcomment_{{$subcomment->id}}">
	    	...
	    	@endif
		    	<p><a class="SubCommentName"><i>{{ $subcomment->getUser($subcomment->user_id)->name }}</i></a>
		    ...
		    <div class="collapse replyComment" id="replySubComment{{$parent}}-{{$subcomment->id}}">
                	<div class="form-group">
                  		...
                  		@if(is_object($currentUser) && $currentUser->id != $subcomment->user_id)
	                        <textarea name="subcomment" class="form-control" rows="3"  id="createSubComment_{{$subcomment->id}}" >@if(is_object($subcomment->getUser($subcomment->user_id))) {{$subcomment->getUser($subcomment->user_id)->name}} @endif</textarea>
                      	@else
