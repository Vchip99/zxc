...
&nbsp;
  ...
  <div class="form-group row">
    <div id="addSubCategoryDiv">
      <a id="addSubCategory" href="{{url('admin/createPayableSubCategory')}}" type="button" class="btn btn-primary" style="float: right;" title="Add New Sub Category">Add New Sub Category</a>&nbsp;&nbsp;
    </div>
  </div>
  <div>
      ...
      <tbody>
        @if(count($testSubCategories)>0)
          ...
          <tr style="overflow: auto;">
            ...
            <td>
              <a href="{{url('admin/payableSubCategory')}}/{{$testSubCategory->id}}/edit" ><img src="{{asset('images/edit1.png')}}" width='30' height='30' title="Edit {{$testSubCategory->name}}" />
                </a>
            </td>
            <td>
                ...
                <form id="deleteSubCategory_{{$testSubCategory->id}}" action="{{url('admin/deletePayableSubCategory')}}" method="POST" style="display: none;">
                    {{ csrf_field() }}
                    {{ method_field('DELETE') }}
          ...
          @endforeach
        @else
          <tr><td colspan="4">No sub category is created.</td></tr>
        @endif
      </tbody>
    </table>
  </div>
