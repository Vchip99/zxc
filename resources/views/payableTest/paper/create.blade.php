...
@section('admin_content')
  ...
  @if(isset($paper->id))
    <form action="{{url('admin/updatePayablePaper')}}" method="POST" id="submitForm">
    {{method_field('PUT')}}
    <input type="hidden" name="paper_id" id="paper_id" value="{{$paper->id}}">
  @else
    <form action="{{url('admin/createPayablePaper')}}" method="POST" id="submitForm">
  @endif
    {{ csrf_field() }}
  ...
  <div class="form-group row @if ($errors->has('name')) has-error @endif">
    ...
    <div class="col-sm-3">
      ...
      <span class="hide" id="paperError" style="color: white;">Given name is already exist with selected sub category and subject.Please enter another name.</span>
    </div>
  </div>
...
<script type="text/javascript">
    ...
    function searchPaper(){
      ...
      if(subcategory && subject && paper){
        $.ajax({
          method:'POST',
          url: "{{url('admin/isPayablePaperExist')}}",
          data:{subcategory:subcategory,subject:subject,paper:paper,paper_id:paperId}
        }).done(function( msg ) {
          if('true' == msg){
