...
@section('module_title')
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      <li><i class="fa fa-credit-card-alt"></i> Online Test </li>
...
@section('admin_content')
	...
	@php
		if(Session::has('payable_selected_subcategory')){
			$selectedSubCategoryId = Session::get('payable_selected_subcategory');
		} else {
			$selectedSubCategoryId = 0;
		}
		if(Session::has('payable_selected_subject')){
			$selectedSubjectId = Session::get('payable_selected_subject');
		} else {
			$selectedSubjectId = 0;
		}
		if(Session::has('payable_selected_paper')){
			$selectedPaperId = Session::get('payable_selected_paper');
		} else {
			$selectedPaperId = 0;
		}
		if(Session::has('payable_selected_section')){
			$selectedSectionId = Session::get('payable_selected_section');
		} else {
			$selectedSectionId = 0;
		}
		if(Session::has('payable_selected_question_type')){
			$selectedQuestionType = Session::get('payable_selected_question_type');
		} else {
			$selectedQuestionType = 1;
		}
		if(Session::has('payable_next_question_no')){
			$nextQuestionNo = Session::get('payable_next_question_no');
		} else {
			$nextQuestionNo = 1;
		}
		if(Session::has('payable_last_common_data')){
			$lastCommonData = Session::get('payable_last_common_data');
		} else {
			$lastCommonData = '';
		}
	@endphp
	<div id="main ">
		<ul id ="ul">
			...
			@endif
	  		...
	  		@if($prevQuestionId > 0)
	  			<li title="Prev Question"><a class="btn" id="prev_ques" href="{{url('admin/payableQuestion')}}/{{$prevQuestionId}}/edit">Prev Question</a></li>
	  		@else
	  			<li title="No Prev Question"><a class="btn" id="prev_ques">No Prev Question</a></li>
	  		@endif
	  		@if($nextQuestionId > 0)
		  		<li title="Next Question"><a class="btn" id="next_ques" href="{{url('admin/payableQuestion')}}/{{$nextQuestionId}}/edit">Next Question</a></li>
	  		@elseif(( $prevQuestionId > 0 || null == $prevQuestionId ) && null == $nextQuestionId )
	  			<li title="Add Question"><a class="btn" id="next_ques" href="{{url('admin/createPayableQuestion')}}">Add Question </a></li>
	  		@else
		...
		@if(isset($testQuestion->id))
			<form id="createForm" name="updateQuestion" action="{{url('admin/updatePayableQuestion')}}" method="POST" role="form" data-toggle="validator">
			{{method_field('PUT')}}
			<input type="hidden" name="question_id" id="question_id" value="{{$testQuestion->id}}">
		@else
			<form id="createForm" name="createQuestion" action="{{url('admin/createPayableQuestion')}}" method="POST">
		@endif
		{{csrf_field()}}
		<div class="container admin_div">
		    ...
		    <div class="form-group row @if ($errors->has('section_type')) has-error @endif">
			    ...
			    <div class="col-sm-3">
			    	@if(isset($testQuestion->id))
			    		@if(count($sessions)>0)
			    			@foreach($sessions as $paperSection)
...
<script type="text/javascript">
	...
	function selectPaper(ele){
  	...
  	function getQuestionCount(categoryId,subcategoryId,paperId,subjectId,section_type){
  		...
  		if( 0 < subcategoryId && 0 < paperId && 0 < subjectId && false == isNaN(questionId)){
	      	$.ajax({
	             	method: "POST",
	              	url: "{{url('admin/getPayableCurrentQuestionCount')}}",
    		...
    		$.ajax({
	             	method: "POST",
	              	url: "{{url('admin/getPayableNextQuestionCount')}}",
  	...
  	function toggleNextPrev(){
  		...
  		if( 0 < subcategoryId && 0 < paperId && 0 < subjectId){
  			$.ajax({
	             	method: "POST",
	              	url: "{{url('admin/getPayablePrevQuestionCount')}}",
	              	data: {categoryId:categoryId,subcategoryId:subcategoryId,paperId:paperId,subjectId:subjectId,section_type:section_type}
          	}).done(function( msg ) {
          		if( 0 < msg ){
		            var prevEle = document.getElementById('prev_ques');
		            prevEle.classList.remove("hide");
		            var url = "{{url('admin/payableQuestion')}}"+'/'+msg+'/edit';
