...
@section('admin_content')
  ...
  @if(isset($vkitCategory->id))
    <form action="{{url('admin/updateVkitCategory')}}" method="POST" id="submitForm">
    {{ method_field('PUT') }}
    <input type="hidden" name="category_id" id="category_id" value="{{$vkitCategory->id}}">
  @else
    <form action="{{url('admin/createVkitCategory')}}" method="POST" id="submitForm">
  @endif
    {{ csrf_field() }}
...
<script type="text/javascript">
  function searchCategory(){
    ...
    if(category){
      $.ajax({
        method:'POST',
        url: "{{url('admin/isVkitCategoryExist')}}",
