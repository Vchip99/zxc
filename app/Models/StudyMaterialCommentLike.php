...
class StudyMaterialCommentLike extends Model
{
    /**
     ...
     * @var array
     */
    protected $fillable = ['study_material_topic_id','study_material_post_id','study_material_comment_id','user_id'];
    	...
    	if(count($postIds) > 0){
	        ...
	        if( false == $likes->isEmpty() ){
	            foreach($likes as $like){
	                $commentLikesCount[$like->study_material_comment_id]['user_id'][$like->user_id] = $like->user_id;
	                $commentLikesCount[$like->study_material_comment_id]['like_id'][$like->id] = $like->id;
	            }
	        }
	    }
    ...
    protected static function getLikeComment(Request $request){
    	...
    	if(is_object($loginUser)){
	    	if( 0 == $request->get('dis_like')){
	    		...
	    		$commentLike  = static::where('study_material_topic_id',$request->get('topic_id'))->where('study_material_post_id', $request->get('post_id'))->where('study_material_comment_id', $request->get('comment_id'))->where('user_id', $loginUser->id)->first();
    ...
    protected static function getCommentStatus(Request $request){
    	return static::where('study_material_topic_id',$request->get('topic_id'))->where('study_material_post_id', $request->get('post_id'))->where('study_material_comment_id', $request->get('comment_id'))->get();
    }
