     ...
     protected static function getLikeVideoComment(Request $request){
        if(is_object(Auth::user())){
            if( 1 == $request->get('dis_like')){
                $likeBlogComment = static::where('live_course_video_id',$request->get('video_id'))->where('user_id' ,Auth::user()->id)->where('live_course_comment_id', $request->get('comment_id'))->first();
                if(is_object($likeBlogComment)){
                    $likeBlogComment->delete();
                    return self::getLikeStatus($request);
                }
            } else {
                static::create(['live_course_video_id' => $request->get('video_id'), 'user_id' => Auth::user()->id, 'live_course_comment_id' => $request->get('comment_id')]);
                return self::getLikeStatus($request);
            }
        }
        return 'false';
