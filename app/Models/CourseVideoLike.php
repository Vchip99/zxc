    ...
    protected static function getLikeVideo(Request $request){
        if(is_object(Auth::user())){
            if( 1 == $request->get('dis_like')){
                $likePost = static::where('course_video_id',$request->get('video_id'))->where('user_id' ,Auth::user()->id)->first();
                if(is_object($likePost)){
                    $likePost->delete();
                    return self::getLikeStatus($request);
                }
            } else {
                static::create(['course_video_id' => $request->get('video_id'), 'user_id' => Auth::user()->id]);
                return self::getLikeStatus($request);
            }
        }
        return 'false';
