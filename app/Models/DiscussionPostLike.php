    ...
    protected static function getLikePost(Request $request){
        if(is_object(Auth::user())){
            if( 1 == $request->get('dis_like')){
                $likePost = static::where('discussion_post_id',$request->get('post_id'))->where('user_id' ,Auth::user()->id)->where( 'is_like', self::IsLike)->first();
                if(is_object($likePost)){
                    $likePost->delete();
                    return self::getLikeStatus($request);
                }
            } else {
                static::create(['discussion_post_id' => $request->get('post_id'), 'user_id' => Auth::user()->id, 'is_like' => self::IsLike]);
                return self::getLikeStatus($request);
            }
        }
        return 'false';
